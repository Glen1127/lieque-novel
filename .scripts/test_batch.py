...
print(f"Calling API with prompt length: {len(prompt)}")
...
print(f"JSON start={start_idx}, end={end_idx}")
if start_idx >= 0 and end_idx > start_idx:
    json_str = response_text[start_idx:end_idx]
    try:
        batch_results = json.loads(json_str)
        print(f"Parsed {len(batch_results)} chapters: {list(batch_results.keys())}")
